...
public:
void Update_item_Rate(long Id,float rate){          //If manager needs to update item_rate then it is used.
struct Inventory *t;
t=Search(Id);
...
void Update_item_Quantity(long Id,long Quantity){  //If manager needs to update item_Quantity then it is used.
struct Inventory *t;
t=Search(Id);
...
nj.Add_item(111100000011,3);//item1,quantity
...
kj.Get_Product_Info(111100000011);
...
//If manager need to update stock rate or update stock quantity than use //update_item_Rate(Id)// and //update_item_Quantity(Id)....//kj.Update_item_Rate(111100000011,300);//
//kj.Update_item_Quantity(111100000011,3);//
